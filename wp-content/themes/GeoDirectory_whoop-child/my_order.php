...
if(($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['post_id'])){// สร้าง order
  ...
  if(!empty($arrProducts))
  {
    ...
    $default_category_id = geodir_get_post_meta( $_POST['post_id'], 'default_category', true );
	...
	list($delivery_fee,$distance) = get_delivery_fee($_POST['post_id']);
    ...
    $wpdb->query(
        $wpdb->prepare(
            "UPDATE orders SET total_amt = %d, shipping_id = %d, billing_id = %d, thread_id = %d where id =%d",
    ...
    $wpdb->query(
      $wpdb->prepare(
        ...
        array($thread_id, $current_user->ID, "ใบสั่งซื้อเลขที่: #".$order_id , '<strong><p style="font-size:14px;">มีรายการสั่งซื้อสินค้าเข้ามาใหม่</p> <a href="'.home_url('/shop-order/').'?pid='.$_POST['post_id'].'">คลิกที่นี่เพื่อดูใบสั่งซื้อ</a></strong>', $current_date)
      )
    );
            ...
            <div class="panel-body">
              ...
              <?php if(!empty($order->driver_adjust)){ 
                        if($order->adjust_accept){
                          ...
                          if($order->status != 99){ 
                            $text = ' ราคาเพิ่มเติมอีก <strong>'.$order->driver_adjust.' บาท</strong> <h4><strong>รวมทั้งหมด '.($order->total_amt+$order->driver_adjust).'</strong> บาท</h4>';
              ...
              <?php if($shipping_price != 0){ ?>        
                <div class="order-row">
                  ...
                  <div class="order-col-3">
                    <strong><?php echo $shipping_price; ?></strong> บาท
                  </div>
                </div>
                <div class="order-clear"></div>
                <hr>
